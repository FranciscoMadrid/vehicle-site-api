...
const BASE_SELECT = `
  SELECT V.id, V.plate, B.brand, M.model, V.created_at, V.fk_model_id, M.fk_brand_id
  FROM vehicle AS V 
...
export const deleteById = async(id) => {
  const sqlLog = 'DELETE FROM log WHERE fk_vehicle_id = ?';
  const resLog = await DB_Query.query(sqlLog, [id])
  const sqlVehicle = `DELETE FROM ${table} WHERE ${primaryKey} = ?`;

  const resVehicle = await DB_Query.query(sqlVehicle, [id]);

  return resVehicle.affectedRows
}